...
function LoginContent() {
  ...
  // 소셜 로그인 버튼 URL — 현재 ?next= 가 있으면 그대로 전달
  const rawNext = searchParams.get('next') || ''
  const safeNext = rawNext.startsWith('/') && !rawNext.startsWith('//') ? rawNext : ''
  const googleHref = safeNext ? `/api/auth/google?next=${encodeURIComponent(safeNext)}` : '/api/auth/google'
  const naverHref = safeNext ? `/api/auth/naver?next=${encodeURIComponent(safeNext)}` : '/api/auth/naver'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 원래 가려던 페이지가 있으면 그쪽으로. 없으면 홈.
      // 외부 URL/프로토콜은 차단(open redirect 방지) — 내부 경로(/...)만 허용.
      const rawNext = searchParams.get('next') || ''
